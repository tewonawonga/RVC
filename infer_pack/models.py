...
from . import commons
from .modules import LayerNorm, ResidualCouplingBlock, ResBlock1, ResBlock2, WN
    ...
    def __init__(self, initial_channel, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, gin_channels, sr, is_half=False):
        ...
        for i, (u, k) in enumerate(zip(upsample_rates, upsample_kernel_sizes)):
            ...
            if i + 1 < len(upsample_rates):
                ...
                self.noise_convs.append(nn.Conv1d(1, c_cur, kernel_size=stride_f0 * 2, stride=stride_f0, padding=stride_f0 // 2))
            else:
...
class SourceModuleHnNSF(nn.Module):
    def __init__(self, sampling_rate, harmonic_num=0, sine_amp=0.1, add_noise_std=0.003, voiced_threshold=0, is_half=False):
        super().__init__()
...
class SineGen(nn.Module):
    ...
    def forward(self, f0, upp):
        with torch.no_grad():
            ...
            rad_values = F.interpolate(rad_values.transpose(2, 1), scale_factor=upp, mode="linear", align_corners=True).transpose(2, 1)
...
class SynthesizerTrnMs256NSFsid(nn.Module):
    ...
    def forward(self, phone, phone_lengths, pitch, pitchf, y, y_lengths, ds):
        ...
        pitchf = commons.slice_segments(pitchf, ids_slice, self.segment_size)
